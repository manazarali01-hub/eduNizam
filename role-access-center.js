(function(){
  ...
  function inject(){
    ...
    sec.innerHTML='<div class="section-head"><div><h2>Access & Role Center</h2><p class="muted">Easy account linking for Head, Teacher, Parent and Student.</p></div><span id="accessRoleBadge" class="badge"></span></div><div class="access-grid"><article class="card" id="claimInviteCard"><h3>Join an Institute</h3><p class="muted">Head se mila invite code enter karein. Teacher/Student/Parent role automatically link ho jayega.</p><div class="form-grid"><input id="claimInviteCode" placeholder="Invite code e.g. EN-AB12CD"><button id="claimInviteBtn">Claim Invite</button></div><div id="claimInviteMsg" class="coverage-note"></div></article><article class="card" id="parentLinkCard"><h3>Parent–Student Link</h3><p class="muted">Parent student ka EduNizam Student Code enter karke access request bhej sakta hai.</p><div class="form-grid"><input id="parentStudentCode" placeholder="Student code"><button id="requestParentLinkBtn">Request Child Link</button></div><div id="parentLinkMsg" class="coverage-note"></div></article><article class="card" id="inviteAdminCard"><div class="section-head"><div><h3>Create Invite Code</h3><p class="muted">Head institute users ke liye simple codes banaye.</p></div></div><div class="form-grid"><select id="inviteRole"><option value="teacher">Teacher</option><option value="student">Student</option><option value="parent">Parent</option></select><input id="inviteUses" type="number" min="1" max="100" value="1" placeholder="Max uses"><input id="inviteDays" type="number" min="1" max="365" value="7" placeholder="Valid days"><button id="createInviteBtn">Generate Invite</button></div><div id="inviteResult"></div><div id="inviteList"></div></article><article class="card" id="parentApprovalCard"><div class="section-head"><div><h3>Parent Access Requests</h3><p class="muted">Pending parent-child links approve ya reject karein.</p></div><button id="refreshParentLinks" class="secondary">Refresh</button></div><div id="accessParentLinks"></div></article></div>';
  ...
  function render(){
    ...
    const admin=document.getElementById('inviteAdminCard'),approval=document.getElementById('parentApprovalCard'),parent=document.getElementById('parentLinkCard');
    if(admin)admin.style.display=r==='head'?'block':'none';if(approval)approval.style.display=r==='head'?'block':'none';if(parent)parent.style.display=r==='parent'?'block':'none';
    if(r==='head'){loadInvites();loadParentLinks()}
  }
  function boot(){
    injectStyle();inject();
