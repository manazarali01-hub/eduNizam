...
as $$
  select jsonb_build_object(
    ...
    'tables', jsonb_build_object(
      ...
      'teacher_training_records', to_regclass('public.teacher_training_records') is not null,
      'school_functions', to_regclass('public.school_functions') is not null,
      'student_spotlight_posts', to_regclass('public.student_spotlight_posts') is not null,
      ...
      'student_behavior_records', to_regclass('public.student_behavior_records') is not null,
      'school_visitors', to_regclass('public.school_visitors') is not null,
      'student_gate_passes', to_regclass('public.student_gate_passes') is not null,
      'lesson_plans', to_regclass('public.lesson_plans') is not null,
      'syllabus_progress_units', to_regclass('public.syllabus_progress_units') is not null,
      'school_helpdesk_tickets', to_regclass('public.school_helpdesk_tickets') is not null,
      'student_parent_complaints', to_regclass('public.student_parent_complaints') is not null,
      'student_parent_complaint_attachments', to_regclass('public.student_parent_complaint_attachments') is not null
    ),
    'functions', jsonb_build_object(
      'current_account_role', to_regprocedure('public.current_account_role()') is not null,
      'can_view_school_notice', to_regprocedure('public.can_view_school_notice(public.school_announcements)') is not null,
      ...
      'create_student_behavior_record', to_regprocedure('public.create_student_behavior_record(uuid,text,text,date,text,text,text,boolean)') is not null,
      'acknowledge_student_behavior_record', to_regprocedure('public.acknowledge_student_behavior_record(uuid)') is not null,
      'create_student_gate_pass', to_regprocedure('public.create_student_gate_pass(uuid,date,time,text,text,text,text)') is not null,
      'update_student_gate_pass_status', to_regprocedure('public.update_student_gate_pass_status(uuid,text,text)') is not null,
      'can_manage_teaching_class', to_regprocedure('public.can_manage_teaching_class(uuid,text,text)') is not null,
      'can_view_family_class', to_regprocedure('public.can_view_family_class(uuid,text,text)') is not null,
      'create_helpdesk_ticket', to_regprocedure('public.create_helpdesk_ticket(uuid,uuid,text,text,text,text)') is not null,
      'update_helpdesk_ticket', to_regprocedure('public.update_helpdesk_ticket(uuid,text,text)') is not null,
      'create_student_parent_complaint', to_regprocedure('public.create_student_parent_complaint(uuid,text,text,text,text)') is not null,
      'acknowledge_student_parent_complaint', to_regprocedure('public.acknowledge_student_parent_complaint(uuid)') is not null,
      'resolve_student_parent_complaint', to_regprocedure('public.resolve_student_parent_complaint(uuid)') is not null
    ),
    'storage', jsonb_build_object(
      'admission_documents_bucket', exists(select 1 from storage.buckets where id='admission-documents'),
      'parent_complaints_bucket', exists(select 1 from storage.buckets where id='parent-complaints'),
      'school_community_media_bucket', exists(select 1 from storage.buckets where id='school-community-media')
    )
  );
$$;
...
-- Require approved institution membership before any account can hold Teacher role.
