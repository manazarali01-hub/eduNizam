...
Submit `https://manazarali01-hub.github.io/eduNizam/sitemap.xml` in Google Search Console after deployment. The repository also includes `robots.txt`, but GitHub project pages serve it under `/eduNizam/robots.txt` instead of the hostname root. Search engines normally request `https://manazarali01-hub.github.io/robots.txt`, so Search Console sitemap submission remains the reliable discovery route until EduNizam uses a custom domain or a root GitHub Pages repository.
