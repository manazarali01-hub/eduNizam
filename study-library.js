(function(){
  ...
  function filters(){return{q:$('studySearch').value.trim().toLowerCase(),board:$('studyBoard').value,cls:$('studyClass').value,subject:$('studySubject').value,type:$('studyType').value,status:$('studyCurriculumStatus')?.value||''}}
  function match(x,f){
    const hay=[x.title,x.board,x.subject,x.type,x.chapter,x.note,x.content].join(' ').toLowerCase();
    return(!f.q||hay.includes(f.q))&&(!f.board||x.board===f.board)&&(!f.cls||x.classLevels.includes(Number(f.cls)))&&(!f.subject||x.subject===f.subject)&&(!f.type||x.type===f.type)&&(!f.status||(x.curriculumStatus||'needs-verification')===f.status)
  }
  function card(x){
    ...
    const reg=window.EDUNIZAM_CURRICULUM_REGISTRY;const auth=reg?.authorities?.find(a=>a.id===x.authorityId);const status=x.curriculumStatus||'needs-verification';const statusBadge=status==='current'?'<span class="trust-badge trust-official">Current Curriculum</span>':status==='archived'?'<span class="trust-badge trust-community">Archived</span>':'<span class="trust-badge trust-community">Needs Verification</span>';return '<article class="paper-card"><div class="paper-card-top"><div><span class="mini-badge">'+esc(x.type)+'</span> '+(x.source==='official'?'<span class="trust-badge trust-official">Official</span>':x.source==='verified'?'<span class="trust-badge trust-verified">Verified</span>':'<span class="trust-badge trust-verified">EduNizam</span>')+' '+statusBadge+'</div><button class="icon-btn" data-study-save="'+x.id+'">'+(isSaved?'★':'☆')+'</button></div><h3>'+esc(x.title)+'</h3><p class="muted">'+esc(x.board)+' · '+esc(x.subject)+' · Class '+esc(x.classLevels.join(', '))+'</p>'+(auth?'<p class="coverage-note"><strong>Authority:</strong> '+esc(auth.name)+(x.curriculumSession?' · <strong>Session:</strong> '+esc(x.curriculumSession):'')+'</p>':'')+(x.note?'<p class="coverage-note">'+esc(x.note)+'</p>':'')+'<div class="paper-actions">'+actions+'</div></article>';
  }
  function render(){
  ...
  $('studyClass').addEventListener('change',()=>{fillSubjects();render()});['studyBoard','studySubject','studyType','studyCurriculumStatus'].forEach(id=>$(id)?.addEventListener('change',render));$('studySearch').addEventListener('input',render);
